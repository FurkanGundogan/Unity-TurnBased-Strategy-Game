...
public class Pathfinding : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (Instance != null)
        {
            Debug.Log($"Instance Error for {nameof(Pathfinding)}");
            Destroy(gameObject);
            return;
        }
        Instance = this;
        ...
        gridSystem = new GridSystem<PathNode>(10, 10, 2f,
            (GridSystem<PathNode> gt, GridPosition gridPosition) => new PathNode(gridPosition));
    ...
    public void Setup(int width, int height, float cellSize)
    {
        ...
        {
            /// cycle all grids
            /// if there is something set node is unwalkable
